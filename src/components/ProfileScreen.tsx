...
const PROFILE_HERO_BACKGROUNDS: Record<WallpaperKey, string> = {
  ...
  vietnam: 'radial-gradient(circle at 16% 12%, rgba(255, 213, 79, 0.78) 0 8%, transparent 9%), radial-gradient(ellipse at 84% 18%, rgba(255, 255, 255, 0.52), transparent 22%), radial-gradient(ellipse at 16% 90%, rgba(16, 185, 129, 0.20), transparent 32%), linear-gradient(135deg, #b91c1c 0%, #ef4444 52%, #f59e0b 100%)',
  korea: 'radial-gradient(circle at 72% 16%, rgba(239, 68, 68, 0.34), transparent 18%), radial-gradient(circle at 28% 20%, rgba(37, 99, 235, 0.34), transparent 20%), radial-gradient(ellipse at 88% 86%, rgba(20, 184, 166, 0.20), transparent 30%), linear-gradient(135deg, #1d4ed8 0%, #60a5fa 48%, #ef4444 120%)',
  futureCat: 'radial-gradient(circle at 50% 16%, rgba(255, 255, 255, 0.74), transparent 18%), radial-gradient(circle at 15% 20%, rgba(14, 165, 233, 0.35), transparent 22%), radial-gradient(circle at 88% 74%, rgba(250, 204, 21, 0.28), transparent 22%), linear-gradient(135deg, #0284c7 0%, #38bdf8 52%, #2563eb 100%)',
};
            ...
            <div className="pf-wallpaper-grid compact" style={{ paddingLeft: 12, paddingRight: 12, paddingBottom: 12 }}>
              {WALLPAPERS.map((w) => (
                <button key={w.key} className={`pf-wallpaper-item ${wallpaper === w.key ? 'active' : ''}`} onClick={() => onChangeWallpaper(w.key)}>
                  <div className={`pf-wallpaper-preview pf-wallpaper-preview--${w.key}`} style={{ background: w.preview }} />
                </button>
              ))}
            </div>
          )}
